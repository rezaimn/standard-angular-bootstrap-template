...
describe ('HeroesService (with spies)', () => {
  ...
  beforeEach(() => {
    ...
    authService = new AuthenticationService(<any> httpClientSpy);
  });
  ...
  it('should return expected heroes (HttpClient called once)', () => {
    ...
    httpClientSpy.post.and.returnValue(asyncData({token:'QpwL5tke4Pnpja7X4'}));

    authService.login('login',loginData).subscribe(
      (token =>{
        expect(token.token).toEqual('QpwL5tke4Pnpja7X4'),
          fail
      })
    );
...
//
//     authService.getHeroes().subscribe(
